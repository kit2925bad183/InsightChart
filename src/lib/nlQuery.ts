...
export type NlAction =
  | { type: "filter-below"; value: number; department?: string }
  | { type: "filter-above"; value: number; department?: string }
  | { type: "filter-range"; min: number; max: number; department?: string }
  | { type: "compare-departments"; departments: string[] }
  | { type: "pie-tiers" }
  | { type: "names-range"; min: number; max: number; department?: string }
  | { type: "unknown"; reason: string };

const NUM = /-?\d+(\.\d+)?/;

function findMentionedDepartment(q: string, knownDepartments: string[]): string | undefined {
  return knownDepartments.find((d) => d && new RegExp(`\\b${d.toLowerCase()}\\b`).test(q));
}
...
export function parseNlQuery(raw: string, knownDepartments: string[]): NlAction {
  ...
  const department = findMentionedDepartment(q, knownDepartments);
  ...
  if (/name/.test(q) && between) {
    return { type: "names-range", min: Number(between[1]), max: Number(between[2]), department };
  }

  if (between) {
    return { type: "filter-range", min: Number(between[1]), max: Number(between[2]), department };
  }

  const belowMatch = q.match(/(below|under|less than|<)\s*(\d+(?:\.\d+)?)/);
  if (belowMatch) return { type: "filter-below", value: Number(belowMatch[2]), department };

  const aboveMatch = q.match(/(above|over|more than|greater than|>)\s*(\d+(?:\.\d+)?)/);
  if (aboveMatch) return { type: "filter-above", value: Number(aboveMatch[2]), department };
